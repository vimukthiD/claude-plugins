...
- **plane** — install per project (`claude plugin install plane@vimukthi-plugins --scope project`), needs Docker and **Plane 1.4.1+**. Reads `PLANE_API_KEY` / `PLANE_WORKSPACE_SLUG` / `PLANE_BASE_URL` from the environment; keep the token in `~/.plane-mcp/env` (`chmod 600`) and set the workspace slug per project. One server process serves exactly one workspace. **Also copy a deny profile** into each project — an unconfigured install exposes all 177 tools including 13 live `delete_*`. Details in [its README](plugins/plane/README.md).
