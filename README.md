...
- **playwright** — install per project (`claude plugin install playwright@vimukthi-plugins --scope project`), fetch the browser binary once per machine (`npx -y @playwright/mcp@0.0.79 install-browser chrome-for-testing`), and add a `browser_run_code_unsafe` **deny** rule to each project that enables it. Details in [its README](plugins/playwright/README.md).
- **plane** — install per project (`claude plugin install plane@vimukthi-plugins --scope project`), needs Docker and **Plane 1.4.1+**. Token and instance URL go in `~/.plane-mcp/env` (`chmod 600`); the workspace goes in a `.plane-workspace` file in each project root. No shell-profile edits — the plugin's own launcher resolves config, and missing config fails loudly at spawn. One server process serves exactly one workspace. **Also copy a deny profile** into each project — an unconfigured install exposes all 177 tools including 13 live `delete_*`. Details in [its README](plugins/plane/README.md).
...
| `dev-kit` | `/plugin install dev-kit@vimukthi-plugins` | My own code, lives in this repo | Four development agents on explicitly pinned models — read-only `architect` on `claude-fable-5`, `builder` on `claude-opus-5`, `operator` on `claude-sonnet-5`, `scout` on `claude-haiku-4-5-20251001` — routed by task type via `/dev-kit:delegate`. Agents and commands only: no hooks, no MCP server, nothing machine-local. See [its README](plugins/dev-kit/README.md). |
| `playwright` | `claude plugin install playwright@vimukthi-plugins --scope project` | Wrapper around Microsoft's `@playwright/mcp`, pinned to `0.0.79` | Browser automation via headless isolated Chromium; meant for per-project activation. `npx`-fetched at first spawn (pinned, but no committed lockfile — see [its README](plugins/playwright/README.md)). |
| `plane` | `claude plugin install plane@vimukthi-plugins --scope project` | Wrapper around Plane's official `makeplane/plane-mcp-server`, pinned to digest `sha256:1feb49a9` (v0.2.11) | Project management against a self-hosted Plane. Runs the upstream Docker image rootless and read-only; requires **Plane 1.4.1+** (1.3.1 breaks `list_projects`). Token read from `~/.plane-mcp/env`, workspace from a per-project `.plane-workspace` file — never committed here, and no shell-profile edits. 92 of its 177 tools are unusable on Plane Community — ships two deny profiles, `read+write` by default. Digest-pinned and commit-traceable but **unsigned upstream**. See [its README](plugins/plane/README.md). |
